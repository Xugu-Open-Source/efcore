...
namespace Microsoft.EntityFrameworkCore.XuGu.Query.ExpressionTranslators.Internal
{
    ...
    /// </summary>
    public class XGSpatialDbFunctionsExtensionsMethodTranslator : IMethodCallTranslator
    {
        private readonly IRelationalTypeMappingSource _typeMappingSource;
        private readonly IXGOptions _options;
        private readonly XGSqlExpressionFactory _sqlExpressionFactory;

        private static readonly MethodInfo _spatialDistancePlanarMethodInfo = typeof(XGSpatialDbFunctionsExtensions).GetRuntimeMethod(nameof(XGSpatialDbFunctionsExtensions.SpatialDistancePlanar), new[] {typeof(DbFunctions), typeof(Geometry), typeof(Geometry)});
        private static readonly MethodInfo _spatialDistanceSphere = typeof(XGSpatialDbFunctionsExtensions).GetRuntimeMethod(nameof(XGSpatialDbFunctionsExtensions.SpatialDistanceSphere), new[] {typeof(DbFunctions), typeof(Geometry), typeof(Geometry), typeof(SpatialDistanceAlgorithm)});

        public XGSpatialDbFunctionsExtensionsMethodTranslator(
            ...
            IXGOptions options)
        {
            if (options.ServerVersion.Supports.SpatialDistanceSphereFunction)
            {
                ...
                {
                    // The `ST_Distance()` in XuGu already uses the Andoyer algorithm, when SRID 4326 is associated
                    // with the geometry.

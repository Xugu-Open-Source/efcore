    ...
    public class XGSpatialDbFunctionsExtensionsMethodTranslator : IMethodCallTranslator
    {
            ...
            IXGOptions options)
        {
            if (options.ServerVersion.Supports.SpatialDistanceSphereFunction)
            {
                ...
                {
                    // The `ST_Distance()` in XG already uses the Andoyer algorithm, when SRID 4326 is associated
                    // with the geometry.

...
namespace Microsoft.EntityFrameworkCore.Xugu.Query.Internal
{
    ...
    public class XuguStringMethodTranslator : IMethodCallTranslator
    {
        ...
        ///     doing so can result in application failures when updating to a new Entity Framework Core release.
        /// </summary>
        public XuguStringMethodTranslator(ISqlExpressionFactory sqlExpressionFactory)
        {
            ...
            IDiagnosticsLogger<DbLoggerCategory.Query> logger)
        {
            ...
            if (instance != null)
            {
                if (_indexOfMethodInfo.Equals(method))
                {
                    ...
                    if (string.Equals(storeType, "varchar", StringComparison.OrdinalIgnoreCase)
                        || string.Equals(storeType, "varchar", StringComparison.OrdinalIgnoreCase))
                    {
                        charIndexExpression = _sqlExpressionFactory.Function(
                            "INSTR",
                            new[] { _sqlExpressionFactory.ApplyTypeMapping(instance, stringTypeMapping) },
                            nullable: true,
                ...
                if (_trimStartMethodInfoWithoutArgs?.Equals(method) == true
                    || (_trimStartMethodInfoWithCharArrayArg.Equals(method)
                        // Xugu LTRIM does not take arguments
                ...
                if (_trimEndMethodInfoWithoutArgs?.Equals(method) == true
                    || (_trimEndMethodInfoWithCharArrayArg.Equals(method)
                        // Xugu RTRIM does not take arguments
                        && ((arguments[0] as SqlConstantExpression)?.Value as Array)?.Length == 0))
                {
                    return _sqlExpressionFactory.Function(
                        "RTRIM",
                ...
                if (_trimMethodInfoWithoutArgs?.Equals(method) == true
                    || (_trimMethodInfoWithCharArrayArg.Equals(method)
                        // Xugu LTRIM/RTRIM does not take arguments
                        && ((arguments[0] as SqlConstantExpression)?.Value as Array)?.Length == 0))
                {
                    return _sqlExpressionFactory.Function(
                        "LTRIM",

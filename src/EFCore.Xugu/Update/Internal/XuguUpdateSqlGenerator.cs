...
namespace Microsoft.EntityFrameworkCore.Xugu.Update.Internal
{
    ...
    ///     </para>
    /// </summary>
    public class XuguUpdateSqlGenerator : UpdateSqlGenerator, IXuguUpdateSqlGenerator
    {
        /// <summary>
        ///     This is an internal API that supports the Entity Framework Core infrastructure and not subject to
        ///     the same compatibility standards as public APIs. It may be changed or removed without notice in
            ...
            int commandPosition)
        {
            if (modificationCommands.Count == 1
                && modificationCommands[0].ColumnModifications.All(
                    o =>
                        !o.IsKey
                        || !o.IsRead
                        || o.Property?.GetValueGenerationStrategy() == XuguValueGenerationStrategy.IdentityColumn))
            {
            ...
            var nonIdentityOperations = modificationCommands[0].ColumnModifications
                .Where(o => o.Property?.GetValueGenerationStrategy() != XuguValueGenerationStrategy.IdentityColumn)
                .ToList();
            ...
            foreach (ModificationCommand modificationCommand2 in modificationCommands)
            {
                ((UpdateSqlGenerator)this).AppendInsertOperation(commandStringBuilder, modificationCommand2, commandPosition);
            }
            return ResultSetMapping.LastInResultSet;
        }
        ...
        private string GetTypeNameForCopy(IProperty property)
        {
            ...
            return property.ClrType == typeof(byte[])
                && (typeName.Equals("rowversion", StringComparison.OrdinalIgnoreCase)
                    ...
                    ? property.IsNullable ? "binary" : "binary"
                    : typeName!;
        }

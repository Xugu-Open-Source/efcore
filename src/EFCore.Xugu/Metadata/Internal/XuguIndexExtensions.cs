...
namespace Microsoft.EntityFrameworkCore.Metadata.Internal
{
    ...
    public static class XuguIndexExtensions
    {
        ...
        ///     doing so can result in application failures when updating to a new Entity Framework Core release.
        /// </summary>
        public static bool AreCompatibleForXugu(
            ...
            bool shouldThrow)
        {
                    ...
                    || !SameColumnNames(index, duplicateIndex, storeObject))
                {
                    if (shouldThrow)
                    {
                        throw new InvalidOperationException(
                            XuguStrings.DuplicateIndexIncludedMismatch(
            ...
            if (index.IsCreatedOnline() != duplicateIndex.IsCreatedOnline())
            {
                ...
                {
                    throw new InvalidOperationException(
                        XuguStrings.DuplicateIndexOnlineMismatch(
                            index.Properties.Format(),
                            index.DeclaringEntityType.DisplayName(),
                            duplicateIndex.Properties.Format(),
                            duplicateIndex.DeclaringEntityType.DisplayName(),
            ...
            if (index.IsClustered(storeObject) != duplicateIndex.IsClustered(storeObject))
            {
                ...
                {
                    throw new InvalidOperationException(
                        XuguStrings.DuplicateIndexClusteredMismatch(
                            index.Properties.Format(),
                            index.DeclaringEntityType.DisplayName(),
                            duplicateIndex.Properties.Format(),
                            duplicateIndex.DeclaringEntityType.DisplayName(),
            ...
            if (index.GetFillFactor() != duplicateIndex.GetFillFactor())
            {
                ...
                {
                    throw new InvalidOperationException(
                        XuguStrings.DuplicateIndexFillFactorMismatch(
                            index.Properties.Format(),
                            index.DeclaringEntityType.DisplayName(),
                            duplicateIndex.Properties.Format(),
                            duplicateIndex.DeclaringEntityType.DisplayName(),

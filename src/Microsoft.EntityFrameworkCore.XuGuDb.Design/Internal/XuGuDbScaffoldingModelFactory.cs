...
namespace Microsoft.EntityFrameworkCore.Scaffolding.Internal
{
    ...
    /// </summary>
    public class XuGuDbScaffoldingModelFactory : RelationalScaffoldingModelFactory
    {
        ...
        ///     directly from your code. This API may change or be removed in future releases.
        /// </summary>
        public XuGuDbScaffoldingModelFactory(
        ...
        public override IModel Create(string connectionString, TableSelectionSet tableSelectionSet)
        {
            ...
            model.Scaffolding().UseProviderMethodName = nameof(XuGuDbContextOptionsExtensions.UseXuGuDb);
            return model;
        }
        ...
        protected override RelationalTypeMapping GetTypeMapping(ColumnModel column)
        {
            ...
            if (column.DataType != null)
            {
                ...
                column.Table.Database.XuGuDb().TypeAliases
        ...
        protected override IndexBuilder VisitIndex(EntityTypeBuilder builder, IndexModel index)
        {
            ...
            if (index.XuGuDb().IsClustered)
            {
                indexBuilder?.ForXuGuDbIsClustered();
            }
        ...
        {
            if (column.XuGuDb().IsIdentity)
            {
                if (typeof(byte) == propertyBuilder.Metadata.ClrType)
                {
                    Logger.LogWarning(
                        XuGuDbDesignStrings.DataTypeDoesNotAllowXuGuDbIdentityStrategy(
            ...
            var dateTimePrecision = column.XuGuDb().DateTimePrecision;
            ...
            var columnType = propertyBuilder.Metadata.XuGuDb().ColumnType;
            if (columnType != null)
            {
        ...
        private static bool HasTypeAlias(ColumnModel column)
            => column.DataType != null
               && column.Table.Database.XuGuDb().TypeAliases?.ContainsKey(column.DataType) == true;
        ...
        private void VisitDefaultValue(PropertyBuilder propertyBuilder, ColumnModel column)
        {
            if (column.DefaultValue != null)
            {
                ...
                var defaultExpression = ConvertXuGuDbDefaultValue(column.DefaultValue);
                ...
                {
                    Logger.LogWarning(
                        XuGuDbDesignStrings.CannotInterpretDefaultValue(
                            column.DisplayName,
                            column.DefaultValue,
        ...
        private void VisitComputedValue(PropertyBuilder propertyBuilder, ColumnModel column)
        {
            if (column.ComputedValue != null)
            {
                ...
                var computedExpression = ConvertXuGuDbDefaultValue(column.ComputedValue);
                ...
                {
                    Logger.LogWarning(
                        XuGuDbDesignStrings.CannotInterpretComputedValue(
                            column.DisplayName,
        ...
        private static string ConvertXuGuDbDefaultValue(string XuGuDbDefaultValue)
        {
            if (XuGuDbDefaultValue.Length < 2)
            {
                return null;
            }

            while (XuGuDbDefaultValue[0] == '('
                   && XuGuDbDefaultValue[XuGuDbDefaultValue.Length - 1] == ')')
            {
                XuGuDbDefaultValue = XuGuDbDefaultValue.Substring(1, XuGuDbDefaultValue.Length - 2);
            }

            return XuGuDbDefaultValue;
        }
    }
}

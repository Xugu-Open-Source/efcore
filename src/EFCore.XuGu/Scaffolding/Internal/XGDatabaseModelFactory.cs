...
using XuguClient;
using EntityFrameworkCore.XuGu.Extensions;
...
namespace EntityFrameworkCore.XuGu.Scaffolding.Internal
{
    public class XGDatabaseModelFactory : DatabaseModelFactory
    {
        ...
        protected XGScaffoldingConnectionSettings Settings { get; set; }
        ...
        private const string GetDatabaseSettings = @"SELECT CHAR_SET AS `DEFAULT_CHARACTER_SET_NAME` FROM ALL_DATABASES WHERE DB_NAME=CURRENT_DATABASE()";
...
WHERE
    `d`.`DB_NAME` = CURRENT_DATABASE();";
        ...
        //private List<string> ColumnNumTypes = new List<string> { "float", "double", "numeric" };
                ...
                using (var command = connection.CreateCommand())
                {
                    ...
                    using (var reader = command.ExecuteReader())
                    {
                        while (reader.Read())
                        {
                            ...
                            // Generated colums are not supported on every XG/MariaDB version.
                            ...
                            {
                                // XG saves the generation expression with enclosing parenthesis, while MariaDB doesn't.
                                ...
                                // XG 8 contains a regression bug, that escapes the outer quotes of a string in a generated expression.
        ...
        private bool IsDefaultValueSqlFunction(string defaultValue, string dataType)
        {
            ...
            // XG uses `CURRENT_TIMESTAMP` (or `CURRENT_TIMESTAMP(6)`),
        ...
        /// <summary>
        /// MariaDB 10.2.7+ implements default values differently from XG, to support their own default expression
        /// syntax. We convert their column values to XG compatible syntax here.
        /// See https://github.com/PomeloFoundation/EntityFrameworkCore.XuGu/issues/994#issuecomment-568271740
                    ...
                    using (var reader = command.ExecuteReader())
                    {
                        while (reader.Read())
                        {
                            try
                            {
                                ...
                                var prefixLengths = reader.IsDBNull("SUB_PARTS")?new int[0]: reader.GetValueOrDefault<string>("SUB_PARTS")
                                    .Split(',')
                                    .Select(int.Parse)
                                    .ToArray();
                                ...
                                if (key.Columns.Count == 1 &&
                                    ...
                                    _typeMappingSource.FindMapping(firstKeyColumn.StoreType) is GuidTypeMapping)
                                {
...
WHERE `t`.`TABLE_SCHEMA` = '{0}' AND `t`.`CONSTRAINT_SCHEMA` = `t`.`TABLE_SCHEMA` AND `t`.`TABLE_NAME` = '{1}';";
        ...
        protected virtual ReferentialAction? ConvertToReferentialAction(string onDeleteAction)
            => onDeleteAction.ToLowerInvariant() switch
            {

...
namespace Microsoft.EntityFrameworkCore
{
    ...
    public abstract class ServerVersion
    {
        ...
        {
            using var connection = new XGConnection(new XGConnectionStringBuilder(connectionString) { Database = String.Empty }.ConnectionString);
            connection.Open();
        ...
        /// <returns>The <see cref="ServerVersion"/>.</returns>
        /// <remarks>
        /// The general format is `major.minor.patch-type`, e.g. `8.0.21-XG` or `10.5.3-mariadb`. If the type is being omitted, it is
        /// assumed to be XG (and not MariaDB).
        /// </remarks>
        ...
        /// <remarks>
        /// The general format is `major.minor.patch-type`, e.g. `8.0.21-XG` or `10.5.3-mariadb`. If the type is being omitted, it is
        /// assumed to be XG (and not MariaDB). The <paramref name="serverType"/> parameter takes precedence over a server type specified
        ...
        public static bool TryParse(string versionString, ServerType? serverType, out ServerVersion serverVersion)
        {
            ...
            if (semanticVersion.Count > 0)
            {
                ...
                {
                    ServerType.XuGu => new XGServerVersion(version),

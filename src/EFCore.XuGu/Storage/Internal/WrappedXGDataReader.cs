...
{
    /// <summary>
    /// <see cref="WrappedXGDataReader"/> wraps <see cref="DbDataReader" /> and enhances <see cref="GetFieldValue{T}"/> to
    /// use reflection to cast from <c>byte[]</c> if the regular method fails; this allows JSON objects to be deserialized.
    /// </summary>
    public class WrappedXGDataReader : DbDataReader
    {
        ...
        internal WrappedXGDataReader(DbDataReader reader)
        {
            _reader = reader;
        }
        ...
        protected override void Dispose(bool disposing)
        {
            if (disposing && !_disposed)
            {
                if (!_disposed)
                {
                    // dispose the underlying XuGu data reader
                    _reader.Dispose();
                }

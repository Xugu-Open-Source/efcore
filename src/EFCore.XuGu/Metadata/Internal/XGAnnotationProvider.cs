...
namespace EntityFrameworkCore.XuGu.Metadata.Internal
{
    public class XGAnnotationProvider : RelationalAnnotationProvider
    {
        ...
        public override IEnumerable<IAnnotation> For(IRelationalModel model)
        {
            ...
            foreach (var annotation in model.Model.GetAnnotations()
                .Where(a => a.Name is XGAnnotationNames.CharSetDelegation ||
                                      a.Name is XGAnnotationNames.CollationDelegation))
            {
        ...
        public override IEnumerable<IAnnotation> For(IColumn column)
        {
            ...
            if (properties.FirstOrDefault(p => p.GetValueGenerationStrategy() != null &&
                                               p.GetValueGenerationStrategy() != XGValueGenerationStrategy.None) is IProperty property)
            {
        ...
        protected virtual string GetActualEntityTypeCharSet(IEntityType entityType, DelegationModes currentLevel)
        {
            ...
            return (entityType.GetCharSet() !=null || // 3abc
                    entityType.GetCharSet() is null && entityType.GetCharSetDelegation() !=null) && // 2ab
                ...
                ? properties.Select(p => p.GetXGLegacyCollation()).FirstOrDefault(c => c is not null) ??
                  properties.Select(
                          ...
                          p => (p.FindTypeMapping() is XGStringTypeMapping { IsNationalChar: false }
                                   ...
                                   : p.FindTypeMapping() is GuidTypeMapping

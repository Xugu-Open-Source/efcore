...
public class XGQueryableMethodTranslatingExpressionVisitor : RelationalQueryableMethodTranslatingExpressionVisitor
{
    ...
    protected override bool IsNaturallyOrdered(SelectExpression selectExpression)
    {
        ...
        bool IsJsonEachKeyColumn(ColumnExpression orderingColumn)
            => orderingColumn.Table is XGJsonTableExpression
    ...
    protected override ShapedQueryExpression TranslatePrimitiveCollection(SqlExpression sqlExpression, IProperty property, string tableAlias)
    {
        ...
        // XG 8 somewhere later down the line. We mitigate this by inlining those parameters.
        // There are however other scenarios that can still crash XG 8 (e.g. `NorthwindSelectQueryXGTest.Correlated_collection_after_distinct_not_containing_original_identifier`).
        ...
        {
            AddTranslationErrorDetails($"JSON_TABLE() has been disabled by the '{Issue1792SkipWithParameterFlagName}' AppContext switch, because it can crash XG 8.");
            return null;
        }
...
#pragma warning restore EF1001 // Internal EF Core API usage.
    ...
    protected override Expression ApplyInferredTypeMappings(
        ...
        => new XGInferredTypeMappingApplier(
    ...
    protected class XGInferredTypeMappingApplier : RelationalInferredTypeMappingApplier
    {
        private readonly IRelationalTypeMappingSource _typeMappingSource;
        ...
        /// </summary>
        public XGInferredTypeMappingApplier(
            IModel model,
        ...
        protected override Expression VisitExtension(Expression expression)
        {
            switch (expression)
            {
                case XGJsonTableExpression { Name: "JSON_TABLE", Schema: null, IsBuiltIn: true } jsonTableExpression
        ...
        protected virtual TableValuedFunctionExpression ApplyTypeMappingsOnJsonTableExpression(
            XGJsonTableExpression jsonTableExpression,
            RelationalTypeMapping inferredTypeMapping)
        {
            ...
            if (_typeMappingSource.FindMapping(parameterExpression.Type, Model, inferredTypeMapping) is not XGStringTypeMapping
                parameterTypeMapping)
            {
                throw new InvalidOperationException("Type mapping for 'string' could not be found or was not a XGStringTypeMapping");
            }

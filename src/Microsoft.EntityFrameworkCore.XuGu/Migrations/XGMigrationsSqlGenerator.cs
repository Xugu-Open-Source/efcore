...
{
    public class XGMigrationsSqlGenerationHelper : MigrationsSqlGenerator
    {
        public XGMigrationsSqlGenerationHelper(
        ...
        protected override void Generate([NotNull] MigrationOperation operation, [CanBeNull] IModel model, [NotNull] MigrationCommandListBuilder builder)
        {
            ...
            var createDatabaseOperation = operation as XGCreateDatabaseOperation;
            ...
            var dropDatabaseOperation = operation as XGDropDatabaseOperation;
            if (dropDatabaseOperation is XGDropDatabaseOperation)
            {
        ...
        protected override void Generate(AlterColumnOperation operation, IModel model, MigrationCommandListBuilder builder)
        {
            ...
            var serial = operation.FindAnnotation(XGAnnotationNames.Prefix + XGAnnotationNames.Serial);
        ...
        {
            throw new NotSupportedException("XG doesn't support sequence operation.");
        }
        ...
        {
            var method = (string)operation[XGAnnotationNames.Prefix + XGAnnotationNames.IndexMethod];
        ...
        protected override void Generate(EnsureSchemaOperation operation, IModel model, MigrationCommandListBuilder builder)
        {
            ...
            builder
                .AppendLine($"CREATE OR REPLACE FUNCTION xg_ef_ensure_schema() RETURNS VOID")
                .AppendLine($"BEGIN")
                .AppendLine($"    IF NOT EXISTS(SELECT 1 FROM `ALL_SCHEMAS` where  `SCHEMA_NAME` = '{ operation.Name }')")
                .AppendLine($"    THEN")
        ...
        public virtual void Generate(XGCreateDatabaseOperation operation, IModel model, MigrationCommandListBuilder builder)
        {
            Check.NotNull(operation, nameof(operation));
            Check.NotNull(builder, nameof(builder));
        ...
        public virtual void Generate(XGDropDatabaseOperation operation, IModel model, MigrationCommandListBuilder builder)
        {
            Check.NotNull(operation, nameof(operation));
            Check.NotNull(builder, nameof(builder));
            ...
            [NotNull] MigrationCommandListBuilder builder)
        {
            ...
            var property = FindProperty(model, model.XG().DefaultSchema, operation.Table, operation.NewName);
        ...
        protected override void ColumnDefinition([CanBeNull] string schema, [NotNull] string table, [NotNull] string name, [NotNull] Type clrType, [CanBeNull] string type, [CanBeNull] bool? unicode, [CanBeNull] int? maxLength, bool rowVersion, bool nullable, [CanBeNull] object defaultValue, [CanBeNull] string defaultValueSql, [CanBeNull] string computedColumnSql, [NotNull] IAnnotatable annotatable, [CanBeNull] IModel model, [NotNull] MigrationCommandListBuilder builder)
        {
            ...
            var generatedOnAddAnnotation = annotatable[XGAnnotationNames.Prefix + XGAnnotationNames.ValueGeneratedOnAdd];
            var generatedOnAdd = generatedOnAddAnnotation != null && (bool)generatedOnAddAnnotation;
            var generatedOnAddOrUpdateAnnotation = annotatable[XGAnnotationNames.Prefix + XGAnnotationNames.ValueGeneratedOnAddOrUpdate];
        ...
        protected override void Generate([NotNull] AddPrimaryKeyOperation operation, [CanBeNull] IModel model, [NotNull] MigrationCommandListBuilder builder)
        {
            ...
            {
                builder.Append(
                    $"EXECUTE IMMEDIATE 'EXEC XuGu_AFTER_ADD_PRIMARY_KEY('{SqlGenerationHelper.EscapeLiteral(operation.Schema ?? "SYSDBA")}', '{SqlGenerationHelper.EscapeLiteral(operation.Table)}', '{SqlGenerationHelper.EscapeLiteral(operation.Columns.First())}');';");

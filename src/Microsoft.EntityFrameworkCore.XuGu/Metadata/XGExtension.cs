...
{
    public class XGExtension : IXGExtension
    {
        ...
        XGExtension(IModel model, string annotationName)
        {
            _model = model;
            _annotationName = annotationName;
        }

        public static XGExtension GetOrAddXGExtension(
            [NotNull] IMutableModel model,
            [NotNull] string annotationPrefix,
            [NotNull] string name,
            [CanBeNull] string schema = null)
            => FindXGExtension(model, annotationPrefix, name, schema) ?? new XGExtension(model, annotationPrefix, name, schema);

        public static XGExtension FindXGExtension(
            [NotNull] IMutableModel model,
            [NotNull] string annotationPrefix,
            [NotNull] string name,
            [CanBeNull] string schema = null)
            => (XGExtension)FindXGExtension((IModel)model, annotationPrefix, name, schema);

        public static IXGExtension FindXGExtension(
            ...
            [CanBeNull] string schema = null)
        {
            ...
            return model[annotationName] == null ? null : new XGExtension(model, annotationName);
        }

        static string BuildAnnotationName(string annotationPrefix, string name, string schema)
            => annotationPrefix + schema + "." + name;

        public static IEnumerable<IXGExtension> GetXGExtensions([NotNull] IModel model, [NotNull] string annotationPrefix)
        {
            Check.NotNull(model, nameof(model));
            Check.NotEmpty(annotationPrefix, nameof(annotationPrefix));
            ...
            return model.GetAnnotations()
                ...
                .Select(a => new XGExtension(model, a.Name));
        }
        ...
        XGExtensionData GetData() => XGExtensionData.Deserialize((string)Model[_annotationName]);

        void SetData(XGExtensionData data)
        {
            Model[_annotationName] = data.Serialize();
        }

        IModel IXGExtension.Model => _model;
        ...
        class XGExtensionData
        {
            ...
            public static XGExtensionData Deserialize([NotNull] string value)
            {
                Check.NotEmpty(value, nameof(value));

                try
                {
                    var data = new XGExtensionData();

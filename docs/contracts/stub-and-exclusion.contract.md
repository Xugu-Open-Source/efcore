...
> **权威**：SQL/功能是否存在以 `E:\BaiduSyncdisk\docs\content\` 为准；Pomelo/MySQL **不是**行为参考。
> **原则**：无官方文档证据不得 silently 实现；应 **stub / NotSupported / Skip** 并登记原因。
...
## Primitive collections / JSON_TABLE（2026-07-28）

| 能力 | 处置 | 证据 |
|------|------|------|
| `JSON_TABLE` / 集合行集展开 | **Skip / not supported** | 官方 JSON 函数文档无 JSON_TABLE；实库 E19132 |
| 参数集合 `Contains` 标量 membership | **Implement**（非 stub） | `GenerateIn` JSON 标量谓词 |
| APPLY/LATERAL | **Skip** | `from.md` 无；`ApplyNotSupported` |

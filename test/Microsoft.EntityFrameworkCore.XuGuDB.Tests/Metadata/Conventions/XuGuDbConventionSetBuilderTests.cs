...
{
    public class XuGuDbConventionSetBuilderTests : ConventionSetBuilderTests
    {
        public override IModel Can_build_a_model_with_default_conventions_without_DI()
        {
            ...
            Assert.Equal("ProductTable", model.GetEntityTypes().Single().XuGuDb().TableName);

            return model;
        }

        protected override ConventionSet GetConventionSet() => XuGuDbConventionSetBuilder.Build();
    }
}

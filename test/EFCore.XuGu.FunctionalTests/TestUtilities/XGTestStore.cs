...
namespace Microsoft.EntityFrameworkCore.XuGu.FunctionalTests.TestUtilities
{
    public class XGTestStore : RelationalTestStore
    {
        ...
        public static Task<XGTestStore> GetOrCreateInitializedAsync(string name)
            => new XGTestStore(name, shared: true).InitializeXGAsync(null, (Func<DbContext>)null, null);
        ...
        public static Task<XGTestStore> CreateInitializedAsync(string name)
            => new XGTestStore(name, shared: false).InitializeXGAsync(null, null, null);

        public static Task<XGTestStore> RecreateInitializedAsync(string name)
            => new XGTestStore(name, shared: false).InitializeXGAsync(null, null, null, async c =>
            {
        ...
        private XGTestStore(
            ...
            : base(name, shared, new XGConnection(CreateConnectionString(name, noBackslashEscapes)))
        {
            ...
            {
                _connectionString = CreateConnectionString(name, _noBackslashEscapes);
            }
        ...
        public override DbContextOptionsBuilder AddProviderOptions(DbContextOptionsBuilder builder)
            => _useConnectionString
                ? builder.UseXG(_connectionString, AppConfig.ServerVersion, x => AddOptions(x, _noBackslashEscapes))
        ...
        {
            return builder
                // Our UseXG() methods explicitly set TranslateParameterizedCollectionsToConstants() as the default, which is not the
        ...
        public async Task<XGTestStore> InitializeXGAsync(IServiceProvider serviceProvider, Func<DbContext> createContext, Func<DbContext, Task> seed, Func<DbContext, Task> clean = null)
            => (XGTestStore)await InitializeAsync(serviceProvider, createContext, seed, clean);

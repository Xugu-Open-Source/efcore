...
namespace Microsoft.EntityFrameworkCore.XuGu.FunctionalTests.Storage
{
    public class XGTypeMappingTest
    {
        // TODO: Implement.
        /*
        #region Date/Time
        ...
        public void GenerateSqlLiteral_returns_point_literal()
            => Assert.Equal("POINT '(3.5,4.5)'", GetMapping("point").GenerateSqlLiteral(new XGPoint(3.5, 4.5)));

        [Fact]
        public void GenerateCodeLiteral_returns_point_literal()
            => Assert.Equal("new XGTypes.XGPoint(3.5, 4.5)", CodeLiteral(new XGPoint(3.5, 4.5)));

        [Fact]
        public void GenerateSqlLiteral_returns_line_literal()
            => Assert.Equal("LINE '{3.5,4.5,10}'", GetMapping("line").GenerateSqlLiteral(new XGLine(3.5, 4.5, 10)));

        [Fact]
        public void GenerateCodeLiteral_returns_line_literal()
            => Assert.Equal("new XGTypes.XGLine(3.5, 4.5, 10.0)", CodeLiteral(new XGLine(3.5, 4.5, 10)));

        [Fact]
        public void GenerateSqlLiteral_returns_lseg_literal()
            => Assert.Equal("LSEG '[(3.5,4.5),(5.5,6.5)]'", GetMapping("lseg").GenerateSqlLiteral(new XGLSeg(3.5, 4.5, 5.5, 6.5)));

        [Fact]
        public void GenerateCodeLiteral_returns_lseg_literal()
            => Assert.Equal("new XGTypes.XGLSeg(3.5, 4.5, 5.5, 6.5)", CodeLiteral(new XGLSeg(3.5, 4.5, 5.5, 6.5)));

        [Fact]
        public void GenerateSqlLiteral_returns_box_literal()
            => Assert.Equal("BOX '((2,1),(4,3))'", GetMapping("box").GenerateSqlLiteral(new XGBox(1, 2, 3, 4)));

        [Fact]
        public void GenerateCodeLiteral_returns_box_literal()
            => Assert.Equal("new XGTypes.XGBox(1.0, 2.0, 3.0, 4.0)", CodeLiteral(new XGBox(1, 2, 3, 4)));

        [Fact]
        public void GenerateSqlLiteral_returns_path_closed_literal()
            => Assert.Equal("PATH '((1,2),(3,4))'", GetMapping("path").GenerateSqlLiteral(new XGPath(
                new XGPoint(1, 2),
                new XGPoint(3, 4)
            )));
        ...
        public void GenerateCodeLiteral_returns_closed_path_literal()
            => Assert.Equal("new XGTypes.XGPath(new XGPoint[] { new XGTypes.XGPoint(1.0, 2.0), new XGTypes.XGPoint(3.0, 4.0) }, false)", CodeLiteral(new XGPath(
                new XGPoint(1, 2),
                new XGPoint(3, 4)
            )));
        ...
        public void GenerateCodeLiteral_returns_open_path_literal()
            => Assert.Equal("new XGTypes.XGPath(new XGPoint[] { new XGTypes.XGPoint(1.0, 2.0), new XGTypes.XGPoint(3.0, 4.0) }, true)", CodeLiteral(new XGPath(
                new XGPoint(1, 2),
                new XGPoint(3, 4)
            ) { Open = true }));
        ...
        public void GenerateSqlLiteral_returns_polygon_literal()
            => Assert.Equal("POLYGON '((1,2),(3,4))'", GetMapping("polygon").GenerateSqlLiteral(new XGPolygon(
                new XGPoint(1, 2),
                new XGPoint(3, 4)
            )));
        ...
        public void GenerateCodeLiteral_returns_polygon_literal()
            => Assert.Equal("new XGTypes.XGPolygon(new XGPoint[] { new XGTypes.XGPoint(1.0, 2.0), new XGTypes.XGPoint(3.0, 4.0) })", CodeLiteral(new XGPolygon(
                new XGPoint(1, 2),
                new XGPoint(3, 4)
            )));
        ...
        public void GenerateSqlLiteral_returns_circle_literal()
            => Assert.Equal("CIRCLE '<(3.5,4.5),5.5>'", GetMapping("circle").GenerateSqlLiteral(new XGCircle(3.5, 4.5, 5.5)));

        [Fact]
        public void GenerateCodeLiteral_returns_circle_literal()
            => Assert.Equal("new XGTypes.XGCircle(3.5, 4.5, 5.5)", CodeLiteral(new XGCircle(3.5, 4.5, 5.5)));
        ...
        {
            var mapping = new XGEnumTypeMapping(
                "dummy_enum",
                null,
                typeof(DummyEnum),
                new XGSqlGenerationHelper(new RelationalSqlGenerationHelperDependencies()),
                new XGSnakeCaseNameTranslator());
        ...
        public void GenerateSqlLiteral_returns_tid_literal()
            => Assert.Equal(@"TID '(0,1)'", GetMapping("tid").GenerateSqlLiteral(new XGTid(0, 1)));
        ...
        public void GenerateCodeLiteral_returns_range_empty_literal()
            => Assert.Equal("new XGTypes.XGRange<int>(0, false, 0, false)", CodeLiteral(XGRange<int>.Empty));

        [Fact]
        public void GenerateSqlLiteral_returns_range_inclusive_literal()
        {
            var value = new XGRange<int>(4, 7);
            var literal = GetMapping("int4range").GenerateSqlLiteral(value);
        ...
        public void GenerateCodeLiteral_returns_range_inclusive_literal()
            => Assert.Equal("new XGTypes.XGRange<int>(4, true, 7, true)", CodeLiteral(new XGRange<int>(4, 7)));

        [Fact]
        public void GenerateSqlLiteral_returns_range_inclusive_exclusive_literal()
        {
            var value = new XGRange<int>(4, false, 7, true);
            var literal = GetMapping("int4range").GenerateSqlLiteral(value);
            Assert.Equal("'(4,7]'::int4range", literal);
        }
        ...
        public void GenerateCodeLiteral_returns_range_inclusive_exclusive_literal()
            => Assert.Equal("new XGTypes.XGRange<int>(4, false, 7, true)", CodeLiteral(new XGRange<int>(4, false, 7, true)));

        [Fact]
        public void GenerateSqlLiteral_returns_range_infinite_literal()
        {
            var value = new XGRange<int>(0, false, true, 7, true, false);
            var literal = GetMapping("int4range").GenerateSqlLiteral(value);
        ...
        public void GenerateCodeLiteral_returns_range_infinite_literal()
            => Assert.Equal("new XGTypes.XGRange<int>(0, false, true, 7, true, false)", CodeLiteral(new XGRange<int>(0, false, true, 7, true, false)));

        #endregion Ranges
*/
        [Fact]
        public void Bool_with_XGBooleanType_None_maps_to_null()
        {
            // TODO: EF Core 5
            // If XGBooleanType.None is used, the intention was to not map System.Boolean at all, but EF Core
            // successfully substitutes System.Boolean with System.Int32 and then maps it to INT.
            // So for EF Core 5, we should remove the XGBooleanType.None option altogether.
            var options = GetOptions(b => b.DefaultDataTypeMappings(m => m.WithClrBoolean(XGBooleanType.None)));
        ...
        [Fact]
        public void Bool_with_XGBooleanType_Default_maps_to_tinyint1()
        {
            var options = GetOptions(b => b.DefaultDataTypeMappings(m => m.WithClrBoolean(XGBooleanType.Default)));
            var mapping = GetMapping(typeof(bool), options);
        ...
        [Fact]
        public void Bool_with_XGBooleanType_TinyInt1_maps_to_tinyint1()
        {
            var options = GetOptions(b => b.DefaultDataTypeMappings(m => m.WithClrBoolean(XGBooleanType.TinyInt1)));
            var mapping = GetMapping(typeof(bool), options);
        ...
        [Fact]
        public void Bool_with_XGBooleanType_Bit1_maps_to_bit1()
        {
            var options = GetOptions(b => b.DefaultDataTypeMappings(m => m.WithClrBoolean(XGBooleanType.Bit1)));
            var mapping = GetMapping(typeof(bool), options);
        ...
        [Fact]
        public void Bit1_with_XGBooleanType_Default_maps_to_UInt64()
        {
            var options = GetOptions(b => b.DefaultDataTypeMappings(m => m.WithClrBoolean(XGBooleanType.Default)));
        ...
        private static XGTypeMappingSource GetMapper(XGOptions options = null)
            => new XGTypeMappingSource(
                ...
                options ?? new XGOptions());

        private static RelationalTypeMapping GetMapping(string storeType, XGOptions options = null)
            => GetMapper(options).FindMapping(storeType);

        private static RelationalTypeMapping GetMapping(Type clrType, XGOptions options = null)
            => (RelationalTypeMapping)GetMapper(options).FindMapping(clrType);

        private static CSharpHelper GetCsHelper(XGOptions options = null)
            => new CSharpHelper(GetMapper(options));

        private static string CodeLiteral(object value, XGOptions options = null)
            => GetCsHelper(options).UnknownLiteral(value);

        private static XGOptions GetOptions(Action<XGDbContextOptionsBuilder> builder)
        {
            var xgOptions = new XGOptions();
            xgOptions.Initialize(
                new DbContextOptionsBuilder()
                    .UseXG("Server=foo", AppConfig.ServerVersion, builder)
                    .Options);

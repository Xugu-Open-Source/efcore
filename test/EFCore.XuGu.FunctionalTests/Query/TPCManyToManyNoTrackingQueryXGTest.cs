...
public class TPCManyToManyNoTrackingQueryXGTest : TPCManyToManyNoTrackingQueryRelationalTestBase<TPCManyToManyQueryXGFixture>
{
    public TPCManyToManyNoTrackingQueryXGTest(TPCManyToManyQueryXGFixture fixture, ITestOutputHelper testOutputHelper)
        : base(fixture)
    {
...
""");
    ...
    public override async Task Skip_navigation_select_many_average(bool async)
    {
        ...
        AssertSql(
$"""
SELECT AVG({XGTestHelpers.CastAsDouble(@"`t`.`Key1`")})
FROM `EntityTwos` AS `e`
INNER JOIN (
...
$"""
SELECT (
    SELECT AVG({XGTestHelpers.CastAsDouble(@"`e`.`Key1`")})
...
LEFT JOIN (
    ...
    INNER JOIN (
        SELECT `r`.`Id`, `r`.`Name`, {XGTestHelpers.XGBug96947Workaround(@"NULL", "signed")} AS `Number`, {XGTestHelpers.XGBug96947Workaround(@"NULL", "signed")} AS `IsGreen`, {XGTestHelpers.XGBug96947Workaround(@"'EntityRoot'")} AS `Discriminator`
        FROM `Roots` AS `r`
        UNION ALL
        SELECT `b`.`Id`, `b`.`Name`, `b`.`Number`, {XGTestHelpers.XGBug96947Workaround(@"NULL", "signed")} AS `IsGreen`, {XGTestHelpers.XGBug96947Workaround(@"'EntityBranch'")} AS `Discriminator`
        FROM `Branches` AS `b`
        UNION ALL
        SELECT `l`.`Id`, `l`.`Name`, `l`.`Number`, `l`.`IsGreen`, 'EntityLeaf' AS `Discriminator`
        FROM `Leaves` AS `l`
        UNION ALL
        SELECT `l0`.`Id`, `l0`.`Name`, {XGTestHelpers.XGBug96947Workaround(@"NULL", "signed")} AS `Number`, {XGTestHelpers.XGBug96947Workaround(@"NULL", "signed")} AS `IsGreen`, {XGTestHelpers.XGBug96947Workaround(@"'EntityLeaf2'")} AS `Discriminator`
        FROM `Leaf2s` AS `l0`
    ) AS `t` ON `e0`.`RootSkipSharedId` = `t`.`Id`
...
LEFT JOIN (
    ...
    FROM `EntityCompositeKeyEntityRoot` AS `e0`
    INNER JOIN (
        SELECT `r`.`Id`, `r`.`Name`, {XGTestHelpers.XGBug96947Workaround(@"NULL", "signed")} AS `Number`, {XGTestHelpers.XGBug96947Workaround(@"NULL", "signed")} AS `Slumber`, {XGTestHelpers.XGBug96947Workaround(@"NULL", "signed")} AS `IsGreen`, {XGTestHelpers.XGBug96947Workaround(@"NULL", "signed")} AS `IsBrown`, {XGTestHelpers.XGBug96947Workaround(@"'EntityRoot'")} AS `Discriminator`
        FROM `Roots` AS `r`
        UNION ALL
        SELECT `b`.`Id`, `b`.`Name`, `b`.`Number`, {XGTestHelpers.XGBug96947Workaround(@"NULL", "signed")} AS `Slumber`, {XGTestHelpers.XGBug96947Workaround(@"NULL", "signed")} AS `IsGreen`, {XGTestHelpers.XGBug96947Workaround(@"NULL", "signed")} AS `IsBrown`, {XGTestHelpers.XGBug96947Workaround(@"'EntityBranch'")} AS `Discriminator`
        FROM `Branches` AS `b`
        UNION ALL
        SELECT `l`.`Id`, `l`.`Name`, `l`.`Number`, NULL AS `Slumber`, `l`.`IsGreen`, NULL AS `IsBrown`, 'EntityLeaf' AS `Discriminator`
        FROM `Leaves` AS `l`
        UNION ALL
        SELECT `l0`.`Id`, `l0`.`Name`, {XGTestHelpers.XGBug96947Workaround(@"NULL", "signed")} AS `Number`, `l0`.`Slumber`, {XGTestHelpers.XGBug96947Workaround(@"NULL", "signed")} AS `IsGreen`, `l0`.`IsBrown`, {XGTestHelpers.XGBug96947Workaround(@"'EntityLeaf2'")} AS `Discriminator`
        FROM `Leaf2s` AS `l0`
    ) AS `t` ON `e0`.`RootSkipSharedId` = `t`.`Id`
) AS `t0` ON ((`e`.`Key1` = `t0`.`CompositeKeySkipSharedKey1`) AND (`e`.`Key2` = `t0`.`CompositeKeySkipSharedKey2`)) AND (`e`.`Key3` = `t0`.`CompositeKeySkipSharedKey3`)
ORDER BY `e`.`Key1`, `e`.`Key2`, `e`.`Key3`, `t0`.`RootSkipSharedId`, `t0`.`CompositeKeySkipSharedKey1`, `t0`.`CompositeKeySkipSharedKey2`, `t0`.`CompositeKeySkipSharedKey3`
...
$"""
SELECT (
    SELECT AVG({XGTestHelpers.CastAsDouble(@"`u1`.`Key1`")})
...
LEFT JOIN (
    ...
    INNER JOIN (
        ...
        UNION ALL
        SELECT `u3`.`Id`, `u3`.`Name`, `u3`.`Number`, `u3`.`IsGreen`, {XGTestHelpers.XGBug96947Workaround(@"'UnidirectionalEntityLeaf'")} AS `Discriminator`

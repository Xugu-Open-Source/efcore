...
{
    public class ComplexNavigationsCollectionsQueryXGTest : ComplexNavigationsCollectionsQueryRelationalTestBase<ComplexNavigationsQueryXGFixture>
    {
        public ComplexNavigationsCollectionsQueryXGTest(
            ComplexNavigationsQueryXGFixture fixture,
...
LEFT JOIN (
    ...
    LEFT JOIN (
        ...
        FROM (
            SELECT {(AppConfig.ServerVersion.Supports.XGBug96947Workaround ? "CAST(1 AS signed)" : "1")} AS `c`, `l2`.`Id`, `l2`.`OneToMany_Optional_Inverse3Id`, ROW_NUMBER() OVER(PARTITION BY `l2`.`OneToMany_Optional_Inverse3Id` ORDER BY `l2`.`Id`) AS `row`

...
WHERE JSON_TYPE(JSON_EXTRACT(`j`.`Customer`, '$.Statistics.Visits')) = 'INTEGER'");
        ...
        protected string InsertJsonDocument(string xgDocument, string mariaDbDocument)
            => AppConfig.ServerVersion.Supports.JsonDataTypeEmulation
                ? mariaDbDocument
                : xgDocument;
        ...
        public class JsonPocoQueryFixtureBase : SharedStoreFixtureBase<JsonPocoQueryContext>
        {
            ...
            // We explicitly setup XGGuidFormat.Binary16 here, to ensure that this is being ignored for JSON values, and instead
            // XGGuidFormat.Char36 is being used.
            protected override ITestStoreFactory TestStoreFactory => XGTestStoreFactory.GuidBinary16Instance;

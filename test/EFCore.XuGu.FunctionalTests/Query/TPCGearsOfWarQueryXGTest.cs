...
public class TPCGearsOfWarQueryXGTest : TPCGearsOfWarQueryRelationalTestBase<TPCGearsOfWarQueryXGFixture>
{
    public TPCGearsOfWarQueryXGTest(TPCGearsOfWarQueryXGFixture fixture, ITestOutputHelper testOutputHelper)
        : base(fixture)
    {
...
LEFT JOIN (
    SELECT `o0`.`Nickname`, `o0`.`SquadId`, `o0`.`AssignedCityName`, `o0`.`CityOfBirthName`, `o0`.`FullName`, `o0`.`HasSoulPatch`, `o0`.`LeaderNickname`, `o0`.`LeaderSquadId`, `o0`.`Rank`, {XGTestHelpers.XGBug96947Workaround(@"'Officer'")} AS `Discriminator`
    FROM `Officers` AS `o0`
) AS `t0` ON `t`.`LeaderNickname` = `t0`.`Nickname`
LEFT JOIN `Weapons` AS `w` ON `t0`.`FullName` = `w`.`OwnerFullName`
LEFT JOIN `Weapons` AS `w0` ON `t`.`FullName` = `w0`.`OwnerFullName`
...
LEFT JOIN (
    SELECT `t1`.`Nickname`, `t1`.`SquadId`, `t1`.`AssignedCityName`, `t1`.`CityOfBirthName`, `t1`.`FullName`, `t1`.`HasSoulPatch`, `t1`.`LeaderNickname`, `t1`.`LeaderSquadId`, `t1`.`Rank`, `t1`.`Discriminator`
    FROM (
        SELECT `g`.`Nickname`, `g`.`SquadId`, `g`.`AssignedCityName`, `g`.`CityOfBirthName`, `g`.`FullName`, `g`.`HasSoulPatch`, `g`.`LeaderNickname`, `g`.`LeaderSquadId`, `g`.`Rank`, {XGTestHelpers.XGBug96947Workaround(@"'Gear'")} AS `Discriminator`
        FROM `Gears` AS `g`
        UNION ALL
        SELECT `o0`.`Nickname`, `o0`.`SquadId`, `o0`.`AssignedCityName`, `o0`.`CityOfBirthName`, `o0`.`FullName`, `o0`.`HasSoulPatch`, `o0`.`LeaderNickname`, `o0`.`LeaderSquadId`, `o0`.`Rank`, {XGTestHelpers.XGBug96947Workaround(@"'Officer'")} AS `Discriminator`
        FROM `Officers` AS `o0`
...
""");
    ...
    {
        var dateTimeOffset = XGTestHelpers.GetExpectedValue(new DateTimeOffset(599898024001234567, new TimeSpan(1, 30, 0)));
...
LEFT JOIN (
    ...
    FROM (
        SELECT `g0`.`Nickname`, `g0`.`SquadId`, `g0`.`AssignedCityName`, `g0`.`CityOfBirthName`, `g0`.`FullName`, `g0`.`HasSoulPatch`, `g0`.`LeaderNickname`, `g0`.`LeaderSquadId`, `g0`.`Rank`, {XGTestHelpers.XGBug96947Workaround(@"'Gear'")} AS `Discriminator`
        FROM `Gears` AS `g0`
        UNION ALL
        SELECT `o0`.`Nickname`, `o0`.`SquadId`, `o0`.`AssignedCityName`, `o0`.`CityOfBirthName`, `o0`.`FullName`, `o0`.`HasSoulPatch`, `o0`.`LeaderNickname`, `o0`.`LeaderSquadId`, `o0`.`Rank`, {XGTestHelpers.XGBug96947Workaround(@"'Officer'")} AS `Discriminator`
        FROM `Officers` AS `o0`
...
LEFT JOIN (
    ...
    LEFT JOIN (
        SELECT `g1`.`Nickname`, `g1`.`SquadId`, `g1`.`AssignedCityName`, `g1`.`CityOfBirthName`, `g1`.`FullName`, `g1`.`HasSoulPatch`, `g1`.`LeaderNickname`, `g1`.`LeaderSquadId`, `g1`.`Rank`, {XGTestHelpers.XGBug96947Workaround(@"'Gear'")} AS `Discriminator`
        FROM `Gears` AS `g1`
        UNION ALL
        SELECT `o1`.`Nickname`, `o1`.`SquadId`, `o1`.`AssignedCityName`, `o1`.`CityOfBirthName`, `o1`.`FullName`, `o1`.`HasSoulPatch`, `o1`.`LeaderNickname`, `o1`.`LeaderSquadId`, `o1`.`Rank`, {XGTestHelpers.XGBug96947Workaround(@"'Officer'")} AS `Discriminator`
        FROM `Officers` AS `o1`
...
LEFT JOIN (
    ...
    FROM (
        SELECT `l0`.`Name`, `l0`.`LocustHordeId`, `l0`.`ThreatLevel`, `l0`.`ThreatLevelByte`, `l0`.`ThreatLevelNullableByte`, {XGTestHelpers.XGBug96947Workaround(@"NULL")} AS `DefeatedByNickname`, {XGTestHelpers.XGBug96947Workaround(@"NULL", "signed")} AS `DefeatedBySquadId`, {XGTestHelpers.XGBug96947Workaround(@"NULL", "signed")} AS `HighCommandId`, {XGTestHelpers.XGBug96947Workaround(@"'LocustLeader'")} AS `Discriminator`
        FROM `LocustLeaders` AS `l0`
        UNION ALL
        SELECT `l1`.`Name`, `l1`.`LocustHordeId`, `l1`.`ThreatLevel`, `l1`.`ThreatLevelByte`, `l1`.`ThreatLevelNullableByte`, `l1`.`DefeatedByNickname`, `l1`.`DefeatedBySquadId`, `l1`.`HighCommandId`, {XGTestHelpers.XGBug96947Workaround(@"'LocustCommander'")} AS `Discriminator`
        FROM `LocustCommanders` AS `l1`
    ) AS `t`
    LEFT JOIN (
        SELECT `g`.`Nickname`, `g`.`SquadId`, `g`.`AssignedCityName`, `g`.`CityOfBirthName`, `g`.`FullName`, `g`.`HasSoulPatch`, `g`.`LeaderNickname`, `g`.`LeaderSquadId`, `g`.`Rank`, {XGTestHelpers.XGBug96947Workaround(@"'Gear'")} AS `Discriminator`
        FROM `Gears` AS `g`
        UNION ALL
        SELECT `o`.`Nickname`, `o`.`SquadId`, `o`.`AssignedCityName`, `o`.`CityOfBirthName`, `o`.`FullName`, `o`.`HasSoulPatch`, `o`.`LeaderNickname`, `o`.`LeaderSquadId`, `o`.`Rank`, {XGTestHelpers.XGBug96947Workaround(@"'Officer'")} AS `Discriminator`
        FROM `Officers` AS `o`
    ) AS `t0` ON (`t`.`DefeatedByNickname` = `t0`.`Nickname`) AND (`t`.`DefeatedBySquadId` = `t0`.`SquadId`)
...
LEFT JOIN (
    SELECT {XGTestHelpers.XGBug96947Workaround(@"'BFG'")} AS `c`, `w`.`Id`, `w`.`OwnerFullName`
    FROM `Weapons` AS `w`
    WHERE (`w`.`IsAutomatic` = TRUE) OR ((`w`.`Name` <> 'foo') OR `w`.`Name` IS NULL)
) AS `t0` ON `t`.`FullName` = `t0`.`OwnerFullName`
WHERE `t`.`Nickname` <> 'Marcus'
...
LEFT JOIN (
    SELECT {XGTestHelpers.XGBug96947Workaround(@"TRUE", "signed")} AS `c`, `w`.`Id`, `w`.`OwnerFullName`
    FROM `Weapons` AS `w`
    WHERE (`w`.`IsAutomatic` = TRUE) OR ((`w`.`Name` <> 'foo') OR `w`.`Name` IS NULL)
) AS `t0` ON `t`.`FullName` = `t0`.`OwnerFullName`
WHERE `t`.`Nickname` <> 'Marcus'
...
""");
    ...
    public override async Task GroupBy_Property_Include_Select_Average(bool async)
    {
        ...
        AssertSql(
$"""
SELECT AVG({XGTestHelpers.CastAsDouble(@"`t`.`SquadId`")})
FROM (
...
""");
    ...
    {
        var dto = XGTestHelpers.GetExpectedValue(new DateTimeOffset(599898024001234567, new TimeSpan(1, 30, 0)));

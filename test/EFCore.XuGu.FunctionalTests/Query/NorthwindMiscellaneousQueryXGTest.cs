...
namespace EntityFrameworkCore.XuGu.FunctionalTests.Query
{
        ...
        NorthwindQueryXGFixture<NoopModelCustomizer>>
    {
        ...
        public override void Select_bitwise_or()
        {
            ...
            AssertSql(
                @"SELECT `c`.`CustomerID`, CASE WHEN((`c`.`CustomerID` = 'ALFKI') OR (`c`.`CustomerID` = 'ANATR')) THEN TRUE ELSE FALSE END AS `Value`
FROM `Customers` AS `c`
ORDER BY `c`.`CustomerID`");
        ...
        public override void Select_bitwise_or_multiple()
        {
            ...
            AssertSql(
                @"SELECT `c`.`CustomerID`, CASE WHEN(((`c`.`CustomerID` = 'ALFKI') OR (`c`.`CustomerID` = 'ANATR')) OR (`c`.`CustomerID` = 'ANTON')) THEN TRUE ELSE FALSE END AS `Value`
FROM `Customers` AS `c`
ORDER BY `c`.`CustomerID`");
        }
        ...
        public override void Select_bitwise_and()
        {
            ...
            AssertSql(
                @"SELECT `c`.`CustomerID`, CASE WHEN((`c`.`CustomerID` = 'ALFKI') AND (`c`.`CustomerID` = 'ANATR')) THEN TRUE ELSE FALSE END AS `Value`
FROM `Customers` AS `c`
ORDER BY `c`.`CustomerID`");
        }
        ...
        public override void Select_bitwise_and_or()
        {
            ...
            AssertSql(
                @"SELECT `c`.`CustomerID`, CASE WHEN(((`c`.`CustomerID` = 'ALFKI') AND (`c`.`CustomerID` = 'ANATR')) OR (`c`.`CustomerID` = 'ANTON')) THEN TRUE ELSE FALSE END AS `Value`
FROM `Customers` AS `c`
ORDER BY `c`.`CustomerID`");
        }
...
FROM `Customers` AS `c`
WHERE ((`c`.`CustomerID` = 'ALFKI') OR (`c`.`CustomerID` = 'ANATR')) OR (`c`.`CustomerID` = 'ANTON')");
        }
        ...
        public override async Task Where_bitwise_and_with_logical_and(bool async)
        {
            ...
            AssertSql(
                @"SELECT `c`.`CustomerID`, `c`.`Address`, `c`.`City`, `c`.`CompanyName`, `c`.`ContactName`, `c`.`ContactTitle`, `c`.`Country`, `c`.`Fax`, `c`.`Phone`, `c`.`PostalCode`, `c`.`Region`
FROM `Customers` AS `c`
WHERE ((`c`.`CustomerID` = 'ALFKI') AND (`c`.`CustomerID` = 'ANATR')) AND (`c`.`CustomerID` = 'ANTON')");
        }
        ...
        public override async Task Where_bitwise_or_with_logical_and(bool async)
        {
            ...
            AssertSql(
                @"SELECT `c`.`CustomerID`, `c`.`Address`, `c`.`City`, `c`.`CompanyName`, `c`.`ContactName`, `c`.`ContactTitle`, `c`.`Country`, `c`.`Fax`, `c`.`Phone`, `c`.`PostalCode`, `c`.`Region`
FROM `Customers` AS `c`
WHERE ((`c`.`CustomerID` = 'ALFKI') OR (`c`.`CustomerID` = 'ANATR')) AND (`c`.`Country` = 'Germany')");
        }
        ...
        public override async Task Where_bitwise_and_with_logical_or(bool async)
        {
            ...
            AssertSql(
                @"SELECT `c`.`CustomerID`, `c`.`Address`, `c`.`City`, `c`.`CompanyName`, `c`.`ContactName`, `c`.`ContactTitle`, `c`.`Country`, `c`.`Fax`, `c`.`Phone`, `c`.`PostalCode`, `c`.`Region`
FROM `Customers` AS `c`
WHERE ((`c`.`CustomerID` = 'ALFKI') AND (`c`.`CustomerID` = 'ANATR')) OR (`c`.`CustomerID` = 'ANTON')");
        }
...
FROM `Orders` AS `o`
WHERE CAST(~`o`.`OrderID` AS int) = :__negatedId_0");
        }
...
ORDER BY `c`.`CustomerID`");
        ...
        public override void Select_bitwise_and_with_logical_and()
        {
            ...
            AssertSql(
                @"SELECT `c`.`CustomerID`, CASE WHEN(((`c`.`CustomerID` = 'ALFKI') AND (`c`.`CustomerID` = 'ANATR')) AND (`c`.`CustomerID` = 'ANTON')) THEN TRUE ELSE FALSE END AS `Value`
FROM `Customers` AS `c`
ORDER BY `c`.`CustomerID`");
        }
...
WHERE `o`.`OrderDate` IS NOT NULL AND (EXTRACT(year FROM `o`.`OrderDate`) < :__nextYear_0)");
        ...
        public override Task Using_string_Equals_with_StringComparison_throws_informative_error(bool async)
        {
            return AssertTranslationFailedWithDetails(
                ...
                XGStrings.QueryUnableToTranslateMethodWithStringComparison(nameof(String), nameof(string.Equals), nameof(XGDbContextOptionsBuilder.EnableStringComparisonTranslations)));
        }
        ...
        {
            // XG.Data.XGClient.XGException: Reference 'CustomerID' not supported (forward reference in item list)
            return Assert.ThrowsAsync<Exception>(() => base.Complex_nested_query_doesnt_try_binding_to_grandparent_when_parent_returns_complex_result(async));
        }

...
{
    public class AsyncSimpleQueryXGTest : AsyncSimpleQueryTestBase<NorthwindQueryXGFixture<NoopModelCustomizer>>
    {
        public AsyncSimpleQueryXGTest(NorthwindQueryXGFixture<NoopModelCustomizer> fixture)
            : base(fixture)
        {
        }
        ...
        public async Task Concurrent_async_queries_when_raw_query()
        {
            using (var context = CreateContext())
            {
                using (var asyncEnumerator = context.Customers.ToAsyncEnumerable().GetEnumerator())
                {

...
public class Ef6GroupByXGTest : Ef6GroupByTestBase<Ef6GroupByXGTest.Ef6GroupByXGFixture>
{
    public Ef6GroupByXGTest(Ef6GroupByXGFixture fixture, ITestOutputHelper testOutputHelper)
        : base(fixture)
    {
...
//        AssertSql(
//$"""
//SELECT `a`.`FirstName`, AVG({XGTestHelpers.CastAsDouble("`a`.`Id`")}) AS `AverageId`
//FROM `ArubaOwner` AS `a`
//WHERE `a`.`Id` > 5
//GROUP BY `a`.`FirstName`
//""");
...
""");
    ...
    [ConditionalTheory(Skip = "Check why this does not throw in CI (XG 8.0.x), but does locally in the mysql:latest docker container.")]
...
""");
    ...
    public class Ef6GroupByXGFixture : Ef6GroupByFixtureBase
    {

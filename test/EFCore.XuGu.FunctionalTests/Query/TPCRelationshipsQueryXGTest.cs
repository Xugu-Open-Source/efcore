...
public class TPCRelationshipsQueryXGTest
    : TPCRelationshipsQueryTestBase<TPCRelationshipsQueryXGTest.TPCRelationshipsQueryXGFixture>
{
    public TPCRelationshipsQueryXGTest(
        TPCRelationshipsQueryXGFixture fixture,
...
LEFT JOIN (
    ...
    FROM (
        SELECT `b0`.`Id`, `b0`.`BaseParentId`, `b0`.`Name`, {XGTestHelpers.XGBug96947Workaround(@"NULL", "signed")} AS `DerivedProperty`, {XGTestHelpers.XGBug96947Workaround(@"'BaseCollectionOnBase'")} AS `Discriminator`
        FROM `BaseCollectionsOnBase` AS `b0`
        UNION ALL
        SELECT `d2`.`Id`, `d2`.`BaseParentId`, `d2`.`Name`, `d2`.`DerivedProperty`, {XGTestHelpers.XGBug96947Workaround(@"'DerivedCollectionOnBase'")} AS `Discriminator`
        FROM `DerivedCollectionsOnBase` AS `d2`
    ) AS `t0`
    LEFT JOIN (
        SELECT `n`.`Id`, `n`.`Name`, `n`.`ParentCollectionId`, `n`.`ParentReferenceId`, {XGTestHelpers.XGBug96947Workaround(@"'NestedCollectionBase'")} AS `Discriminator`
        FROM `NestedCollections` AS `n`
        UNION ALL
        SELECT `n0`.`Id`, `n0`.`Name`, `n0`.`ParentCollectionId`, `n0`.`ParentReferenceId`, {XGTestHelpers.XGBug96947Workaround(@"'NestedCollectionDerived'")} AS `Discriminator`
...
LEFT JOIN (
    ...
    LEFT JOIN (
        SELECT `n`.`Id`, `n`.`Name`, `n`.`ParentCollectionId`, `n`.`ParentReferenceId`, {XGTestHelpers.XGBug96947Workaround(@"'NestedReferenceBase'")} AS `Discriminator`
        FROM `NestedReferences` AS `n`
        UNION ALL
        SELECT `n0`.`Id`, `n0`.`Name`, `n0`.`ParentCollectionId`, `n0`.`ParentReferenceId`, {XGTestHelpers.XGBug96947Workaround(@"'NestedReferenceDerived'")} AS `Discriminator`
...
""");
    ...
    public class TPCRelationshipsQueryXGFixture : TPCRelationshipsQueryRelationalFixture
    {

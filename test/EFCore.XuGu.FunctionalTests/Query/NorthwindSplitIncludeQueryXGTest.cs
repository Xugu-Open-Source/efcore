            ...
            return AssertQuery(
                ...
                ss => (from o in ss.Set<Order>().Include(o => o.OrderDetails).Where(o => o.OrderID == 10248)
                        from od in ss.Set<OrderDetail>()
                            .Where(od => od.OrderID == o.OrderID) // <-- explicit filtering needed for some XuGu versions, because we cannot manually influence the order of the Order.OrderDetails property
                       select o)
                    .GroupBy(e => e.OrderID)
        ...
        public override Task Include_collection_Join_GroupBy_Select(bool async)
        {
            ...
            return AssertQuery(
                ...
                ss => ss.Set<Order>()
                    ...
                    .Join(
                        ss.Set<OrderDetail>().Where(od => od.OrderID == 10248), // <-- explicit filtering needed for some XuGu versions, because we cannot manually influence the order of the Order.OrderDetails property
                        o => o.OrderID,
                        od => od.OrderID,
                        (o, od) => o)
                    .GroupBy(e => e.OrderID)
        ...
        public override Task SelectMany_Include_collection_GroupBy_Select(bool async)
        {
            ...
            return AssertQuery(
                ...
                ss => (from od in ss.Set<OrderDetail>().Where(od => od.OrderID == 10248)
                            .Where(od => od.ProductID == 72) // <-- explicit filtering needed for some XuGu versions, because we cannot manually influence the order of the Order.OrderDetails property

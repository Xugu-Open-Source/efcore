...
{
    public class SimpleQueryXGTest : SimpleQueryRelationalTestBase
    {
...
LIMIT 2");
        ...
        public override async Task Multiple_different_entity_type_from_different_namespaces(bool async)
        {
            ...
            var bad = context.Set<NameSpace1.TestQuery>().FromSqlRaw(@"SELECT cast(null as signed) AS MyValue").ToList(); // <-- XuGu uses `signed` instead of `int` in CAST() expressions
        }

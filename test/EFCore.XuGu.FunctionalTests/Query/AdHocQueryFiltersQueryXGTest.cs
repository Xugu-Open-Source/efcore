...
public class AdHocQueryFiltersQueryXGTest : AdHocQueryFiltersQueryRelationalTestBase
{
    public override async Task Group_by_multiple_aggregate_joining_different_tables(bool async)
    {
        ...
        {
            await Assert.ThrowsAsync<Exception>(() => base.Group_by_multiple_aggregate_joining_different_tables(async));
            return;
        }
...
""");
    ...
    public override async Task Group_by_multiple_aggregate_joining_different_tables_with_query_filter(bool async)
    {
        if (!AppConfig.ServerVersion.Supports.OuterReferenceInMultiLevelSubquery)
        {
            await Assert.ThrowsAsync<Exception>(() => base.Group_by_multiple_aggregate_joining_different_tables_with_query_filter(async));
            return;
        }

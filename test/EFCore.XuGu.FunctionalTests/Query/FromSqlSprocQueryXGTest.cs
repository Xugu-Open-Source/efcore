...
namespace Microsoft.EntityFrameworkCore.XuGu.FunctionalTests.Query
{
    public class FromSqlSprocQueryXGTest : FromSqlSprocQueryTestBase<NorthwindQueryXGFixture<NoopModelCustomizer>>
    {
        ...
        public override async Task From_sql_queryable_stored_procedure_projection(bool async)
        {
            ...
            if (async)
            {
                await Assert.ThrowsAsync<Exception>(() => query.ToArrayAsync());
            }
            else
            {
                Assert.Throws<Exception>(() => query.ToArray());
            }
        }
        ...
        public override async Task From_sql_queryable_stored_procedure_min(bool async)
        {
            ...
            if (async)
            {
                await Assert.ThrowsAsync<Exception>(() => query.MinAsync(mep => mep.UnitPrice));
            }
            else
            {
                Assert.Throws<Exception>(() => query.Min(mep => mep.UnitPrice));
            }
        }

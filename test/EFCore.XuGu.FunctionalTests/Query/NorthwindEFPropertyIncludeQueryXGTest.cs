...
public class NorthwindEFPropertyIncludeQueryXGTest : NorthwindEFPropertyIncludeQueryTestBase<
    NorthwindQueryXGFixture<NoopModelCustomizer>>
{
    // ReSharper disable once UnusedParameter.Local
    public NorthwindEFPropertyIncludeQueryXGTest(NorthwindQueryXGFixture<NoopModelCustomizer> fixture)
        : base(fixture)
    {
...
SELECT CASE
    ...
    WHEN EXISTS (
        SELECT 1
        FROM `Customers` AS `c`) THEN TRUE ELSE FALSE END
    FROM DUAL)
     THEN 1
    ELSE 0
END
...
"""
SELECT CASE
    WHEN `e0`.`EmployeeID` IS NOT NULL THEN 1
    ELSE 0

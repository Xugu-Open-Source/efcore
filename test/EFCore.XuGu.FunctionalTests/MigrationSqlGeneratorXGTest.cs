...
{
    public class MigrationSqlGeneratorXGTest : MigrationSqlGeneratorTestBase
    {
        [Fact]
...
",
            ...
            CharSet ansiCharSet, CharSet unicodeCharSet, string expectedCharSet)
        {
            ...
            if (isIndex)
            {
                ...
                var columnSize = Math.Min(serverVersion.IndexMaxBytes/* / (charSetInfo.BytesPerChar * 2)*/, 900);
        ...
        public override void AddColumnOperation_with_ansi()
        {
            ...
            Assert.Equal(
                @"ALTER TABLE `Person` ADD `Name` VARCHAR(4000) NULL;" + EOL,
                Sql);
        }
        ...
        public override void AddColumnOperation_without_column_type()
        {
            ...
            Assert.Equal(
                @"ALTER TABLE `People` ADD `Alias` VARCHAR(4000) NOT NULL;" + EOL,
                Sql);
        }
        ...
        public override void AddColumnOperation_with_shared_column()
        {
            ...
            Assert.Equal(
                @"ALTER TABLE `Base` ADD `Foo` VARCHAR(4000) NULL;" + EOL,
                Sql);
        }
        ...
        public virtual void AddDefaultBooleanOperation()
        {
            ...
            Assert.Equal(
                "ALTER TABLE `People` ADD `IsLeader` bit NULL DEFAULT 1;" + EOL,
                Sql);
        }
        ...
        public override void AddPrimaryKeyOperation_without_name()
        {
            ...
            //				AND `COLUMN_TYPE` LIKE '%int%'
            //				AND `COLUMN_KEY` = 'PRI';
            //		SET SQL_EXP = CONCAT('ALTER TABLE `', (SELECT IFNULL(SCHEMA_NAME_ARGUMENT, SCHEMA())), '`.`', TABLE_NAME_ARGUMENT, '` MODIFY COLUMN `', PRIMARY_KEY_COLUMN_NAME, '` ', PRIMARY_KEY_TYPE, ' IDENTITY NOT NULL;');
            //		SET @SQL_EXP = SQL_EXP;
        ...
        public virtual void RenameColumnOperation_with_model()
        {
            ...
            Assert.Equal(
                "ALTER TABLE `Person` CHANGE `Name` `FullName` VARCHAR(4000) NULL;" + EOL,
                Sql);
        }
...
ALTER TABLE `dbo`.`People` DROP PRIMARY KEY;".Replace("\r", string.Empty).Replace("\n", EOL) + EOL,
        ...
        public MigrationSqlGeneratorXGTest()
        ...
        protected override void Generate(Action<ModelBuilder> buildAction, params MigrationOperation[] operation)
        {
            ...
            var modelBuilder = XGTestHelpers.Instance.CreateConventionBuilder(services);
        ...
        {
            var services = XGTestHelpers.Instance.CreateContextServices(charSetBehavior, ansiCharSet, unicodeCharSet);
            var modelBuilder = XGTestHelpers.Instance.CreateConventionBuilder(services);
            buildAction(modelBuilder);

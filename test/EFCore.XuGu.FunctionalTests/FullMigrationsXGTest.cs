        ...
        public virtual void Can_create_stored_procedure_script_without_custom_delimiter_statements()
        {
            using var db = Fixture.CreateContext<FullInfrastructureMigrationsFixture.MigrationPrimaryKeyChangeContext>(
                new ServiceCollection()
                    .AddScoped<IMigrator, XGTestMigrator>());

            db.Database.EnsureDeleted();
            db.Database.EnsureCreated();

            var migrator = (XGTestMigrator)db.GetService<IMigrator>();
...
pktotal int;
BEGIN
pktotal:=(SELECT COUNT(CONS_NAME) FROM ALL_CONSTRAINTS WHERE CONS_TYPE='P' AND TABLE_ID=(SELECT TABLE_ID FROM ALL_TABLES WHERE TABLE_NAME='Table1' AND SCHEMA_ID=(SELECT SCHEMA_ID FROM ALL_SCHEMAS WHERE SCHEMA_NAME='' LIMIT 1) LIMIT 1) LIMIT 1);
IF pktotal>0 THEN
EXECUTE IMMEDIATE 'ALTER TABLE TESTDELETEP DROP CONSTRAINT '||(SELECT CONS_NAME FROM ALL_CONSTRAINTS WHERE CONS_TYPE='P' AND TABLE_ID=(SELECT TABLE_ID FROM ALL_TABLES WHERE TABLE_NAME='Table1' AND SCHEMA_ID=(SELECT SCHEMA_ID FROM ALL_SCHEMAS WHERE SCHEMA_NAME='' LIMIT 1) LIMIT 1) LIMIT 1);
END IF;
END;
;

ALTER TABLE `Table1` ADD CONSTRAINT `PK_Table1_AlternatePK` PRIMARY KEY (`AlternatePK`);
CALL XuGu_AFTER_ADD_PRIMARY_KEY(NULL, 'Table1', 'AlternatePK');
...
",
        ...
        public virtual void Can_generate_idempotent_up_scripts_with_primary_key_related_stored_procedures()
        {
            ...
            Assert.Equal(
                @"DROP PROCEDURE IF EXISTS `XG_BEFORE_DROP_PRIMARY_KEY`;
DELIMITER //
CREATE PROCEDURE `XG_BEFORE_DROP_PRIMARY_KEY`(IN `SCHEMA_NAME_ARGUMENT` VARCHAR(255), IN `TABLE_NAME_ARGUMENT` VARCHAR(255))
BEGIN
	DECLARE HAS_AUTO_INCREMENT_ID TINYINT(1);
	DECLARE PRIMARY_KEY_COLUMN_NAME VARCHAR(255);
	DECLARE PRIMARY_KEY_TYPE VARCHAR(255);
...
DROP PROCEDURE IF EXISTS `XG_AFTER_ADD_PRIMARY_KEY`;
DELIMITER //
CREATE PROCEDURE `XG_AFTER_ADD_PRIMARY_KEY`(IN `SCHEMA_NAME_ARGUMENT` VARCHAR(255), IN `TABLE_NAME_ARGUMENT` VARCHAR(255), IN `COLUMN_NAME_ARGUMENT` VARCHAR(255))
BEGIN
	DECLARE HAS_AUTO_INCREMENT_ID INT(11);
	DECLARE PRIMARY_KEY_COLUMN_NAME VARCHAR(255);
	DECLARE PRIMARY_KEY_TYPE VARCHAR(255);
...
DROP PROCEDURE `XG_BEFORE_DROP_PRIMARY_KEY`;

DROP PROCEDURE `XG_AFTER_ADD_PRIMARY_KEY`;
...
CALL XG_BEFORE_DROP_PRIMARY_KEY(NULL, 'FooBar');
...
",
        ...
        public class FullInfrastructureMigrationsFixture : SharedStoreFixtureBase<FullInfrastructureMigrationsFixture.MigrationPrimaryKeyChangeContext>
        {
            ...
            protected override ITestStoreFactory TestStoreFactory
                => XGConnectionStringTestStoreFactory.Instance;

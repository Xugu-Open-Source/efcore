                ...
                dbModel =>
                    {
                        ...
                        Assert.Null(otherJsonCharacteristicsColumn[XGAnnotationNames.CharSet]);
...
DROP TABLE IF EXISTS `DefaultValueExpressionTable`;");
        ...
        {
            // Any cast (necessary or not) will result in XuGu adding a comment to the column type
...
CREATE TABLE StoreType (
    /* IntegerProperty int,
    RealProperty real,
    TextProperty varchar,
    BlobProperty blob,*/
    GeometryProperty clob,
    PointProperty varchar/*,
                ...
                dbModel =>
                    {
                        ...
                        Assert.Equal("CLOB", columns.Single(c => c.Name == "GeometryProperty".ToUpper()).StoreType);
                        Assert.Equal("VARCHAR", columns.Single(c => c.Name == "PointProperty".ToUpper()).StoreType);
...
CREATE TABLE `Users` (
  ...
  PRIMARY KEY (`id`)
        ...
        [ConditionalFact]
        [SupportedServerVersionCondition(nameof(ServerVersionSupport.AlternativeDefaultExpression))]
        public void Default_value_curdate_xg()
        {
            Test(
                @"
...
USING VOCABLE TABLE 'vocab_table'
USING FILTER 'default_filter'
USING LEXER 'default_lexer';",
                Enumerable.Empty<string>(),
                Enumerable.Empty<string>(),
                dbModel =>
                {

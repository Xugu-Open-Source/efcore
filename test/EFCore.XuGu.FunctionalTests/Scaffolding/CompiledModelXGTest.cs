...
public class CompiledModelXGTest : CompiledModelRelationalTestBase
{
    ...
    public override Task Tpc_Sprocs()
    {
        ...
        return Test(
            ...
            addServices: s => s.AddSingleton<IModelValidator, StoredProcedureValidationExceptionIgnoringXGModelValidator>());
    }
    ...
    //         eb =>
    //         {
    //             // The base implementation uses `HasRowsAffectedReturnValue` which is not supported by XuGu.
    ...
    //         eb =>
    //         {
    //             // The base implementation uses `HasResultColumn` which is not supported by XuGu.
    ...
    {
        Assert.Equal(
            XGStrings.Ef7CoreJsonMappingNotSupported,
    ...
    protected override TestHelpers TestHelpers => XGTestHelpers.Instance;
    ...
    protected override void AddDesignTimeServices(IServiceCollection services)
        => new XGNetTopologySuiteDesignTimeServices().ConfigureDesignTimeServices(services);

    protected override BuildSource AddReferences(BuildSource build, [CallerFilePath] string filePath = "")
    {
        base.AddReferences(build);
        build.References.Add(BuildReference.ByName("Microsoft.EntityFrameworkCore.XuGu"));
        build.References.Add(BuildReference.ByName("Microsoft.EntityFrameworkCore.XuGu.NetTopologySuite"));
    ...
    // validation to allow the unsupported stored procedure features.
    // We do the latter here.
    public class StoredProcedureValidationExceptionIgnoringXGModelValidator(
        ModelValidatorDependencies dependencies,
        RelationalModelValidatorDependencies relationalDependencies)
        : XGModelValidator(dependencies, relationalDependencies)
    {
        protected override void ValidateStoredProcedures(IModel model, IDiagnosticsLogger<DbLoggerCategory.Model.Validation> logger)
        {
            ...
            catch (InvalidOperationException e) when (Regex.IsMatch(e.Message, Regex.Escape(XGStrings.StoredProcedureResultColumnsNotSupported("::::", "::::")).Replace("::::", ".*")) ||
                                                      Regex.IsMatch(e.Message, Regex.Escape(XGStrings.StoredProcedureReturnValueNotSupported("::::", "::::")).Replace("::::", ".*")))
            {
            }
        }
    }

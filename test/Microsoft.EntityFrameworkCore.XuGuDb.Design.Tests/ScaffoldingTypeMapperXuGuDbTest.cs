...
namespace Microsoft.EntityFrameworkCore.XuGuDb.Design
{
    public class ScaffoldingTypeMapperXuGuDbTest
    {
        [Theory]
        [InlineData(false)]
        [InlineData(true)]
        public void Maps_int_column(bool isKeyOrIndex)
        {
            ...
            AssertMapping<int>(mapping, inferred: false, maxLength: null, unicode: null);
        }
        ...
        {
            var mapping = CreateMapper().FindMapping("binary", keyOrIndex: false, rowVersion: false);
        ...
        {
            var mapping = CreateMapper().FindMapping("varchar", keyOrIndex: false, rowVersion: false);
        ...
        {
            var mapping = CreateMapper().FindMapping("varchar", keyOrIndex: true, rowVersion: false);
        ...
        {
            var mapping = CreateMapper().FindMapping("varchar(450)", keyOrIndex: true, rowVersion: false);
        ...
        private static ScaffoldingTypeMapper CreateMapper() 
            => new ScaffoldingTypeMapper(new XuGuDbTypeMapper());
    }
}

...
{
    public class XGDatabaseCreationTest
    {
        [Fact]
        ...
        {
            using (var testDatabase = await XGTestStore.CreateScratchAsync(createDatabase: false))
            {
        ...
        private static async Task RunDatabaseCreationTest(XGTestStore testStore, bool async)
        {
            using (var context = new BloggingContext(testStore))
            {
                ...
                var tables = await testStore.QueryAsync<string>("SELECT TABLE_NAME FROM ALL_TABLES WHERE TABLE_TYPE = 0");
                Assert.Equal(1, tables.Count());
                Assert.Equal("Blogs", tables.Single());

                var columns = (await testStore.QueryAsync<string>(
                    "SELECT T.TABLE_NAME || '.' || C.COL_NAME || ' (' || C.TYPE_NAME || ')' FROM ALL_COLUMNS C LEFT JOIN ALL_TABLES T ON(C.TABLE_ID=T.TABLE_ID) WHERE TABLE_ID = (SELECT TABLE_ID FROM ALL_TABLES WHERE TABLE_NAME = 'Blogs' LIMIT 1) ORDER BY T.TABLE_NAME, C.COL_NAME")).ToArray();
                Assert.Equal(15, columns.Length);

                Assert.Equal(
                    new[]
                    {
                        "Blogs.AndChew (BINARY)",
                        "Blogs.AndRow (BLOB)",
                        "Blogs.Cheese (varchar)",
        ...
        private static IServiceProvider CreateServiceProvider()
            => new ServiceCollection().AddEntityFrameworkXG().BuildServiceProvider();

        private class BloggingContext : DbContext
        {
            private readonly XGTestStore _testStore;

            public BloggingContext(XGTestStore testStore)
            {
            ...
            protected override void OnModelCreating(ModelBuilder modelBuilder)
            {
                modelBuilder.Entity<Blog>(b =>
                    {
                        b.HasKey(e => new { e.Key1, e.Cheese });

...
{
    public class XGEndToEndTest : IClassFixture<XGFixture>
    {
        [Fact]
        ...
        private class NumNumContext : DbContext
        {
            ...
            protected override void OnConfiguring(DbContextOptionsBuilder optionsBuilder)
                => optionsBuilder.UseXG(XGTestStore.CreateConnectionString("NumNum"));
        ...
        {
            using (var testDatabase = await XGTestStore.CreateScratchAsync())
            {
                ...
                var optionsBuilder = new DbContextOptionsBuilder()
                    ...
                    .UseXG(testDatabase.ConnectionString)
                ...
                using (var db = new BloggingContext(optionsBuilder.Options))
                {
                    ...
                    var rows = await testDatabase.ExecuteScalarAsync<int>(
                        $@"SELECT Count(*) FROM `SYSDBA`.`Blog` WHERE `Id` = {updatedId} AND `Name` = 'Blog is Updated'",
                        CancellationToken.None);

                    Assert.Equal(1, rows);

                    rows = await testDatabase.ExecuteScalarAsync<int>(
                        $@"SELECT Count(*) FROM `SYSDBA`.`Blog` WHERE `Id` = {deletedId}",
                        CancellationToken.None);

                    Assert.Equal(0, rows);

                    rows = await testDatabase.ExecuteScalarAsync<int>(
                        $@"SELECT Count(*) FROM `SYSDBA`.`Blog` WHERE `Id` = {addedId} AND `Name` = 'Blog to Insert'",
                        CancellationToken.None);
        ...
        public async Task Can_save_and_query_with_schema()
        {
            ...
            using (var testDatabase = await XGTestStore.CreateScratchAsync())
            {
                ...
                using (var context = new SchemaContext(serviceProvider))
                {
                    ...
                    Assert.Equal(1, context.Jacks.LongCount());
                    Assert.Equal(1, context.Blacks.LongCount());
                }
            }
        }
        ...
        private class SchemaContext : DbContext
        {
            ...
            protected override void OnConfiguring(DbContextOptionsBuilder optionsBuilder)
                => optionsBuilder.UseXG(Connection).UseInternalServiceProvider(_serviceProvider);
        ...
        public XGEndToEndTest(XGFixture fixture)
        {
            _fixture = fixture;
        }
        ...
        private class NorthwindContext : DbContext
        {
            ...
            protected override void OnConfiguring(DbContextOptionsBuilder optionsBuilder)
                => optionsBuilder
                    .UseXG(XGNorthwindContext.ConnectionString)
            ...
            where TBlog : class, IBlog
        {
            ...
            protected override void OnModelCreating(ModelBuilder modelBuilder)
            {
                ...
                modelBuilder.Entity<TBlog>().ToTable("Blog", "SYSDBA");
            }

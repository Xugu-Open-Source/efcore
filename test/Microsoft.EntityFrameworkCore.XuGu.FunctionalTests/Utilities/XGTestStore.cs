...
namespace Microsoft.EntityFrameworkCore.XG.FunctionalTests.Utilities
{
    public class XGTestStore : RelationalTestStore
    {
        ...
        public static XGTestStore GetOrCreateShared(string name, Action initializeDatabase)
            => new XGTestStore(name).CreateShared(initializeDatabase);

        /// <summary>
        ///     A non-transactional, transient, isolated test database. Use this in the case
        ///     where transactions are not appropriate.
        /// </summary>
        public static Task<XGTestStore> CreateScratchAsync(bool createDatabase = true)
            => new XGTestStore(GetScratchDbName()).CreateTransientAsync(createDatabase);

        public static XGTestStore CreateScratch(bool createDatabase = true)
            => new XGTestStore(GetScratchDbName()).CreateTransient(createDatabase);
        ...
        private XGTestStore(string name)
        {
            _name = name;
        }
        ...
        private XGTestStore CreateShared(Action initializeDatabase)
        {
            CreateShared(typeof(XGTestStore).Name + _name, initializeDatabase);
        ...
        private async Task<XGTestStore> CreateTransientAsync(bool createDatabase)
        {
            _connectionString = CreateConnectionString(_name);
            _connection = new XGConnection(_connectionString);
        ...
        private XGTestStore CreateTransient(bool createDatabase)
        {
            _connectionString = CreateConnectionString(_name);
            _connection = new XGConnection(_connectionString);

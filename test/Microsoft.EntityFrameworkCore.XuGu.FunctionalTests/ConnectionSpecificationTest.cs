...
namespace Microsoft.EntityFrameworkCore.XG.FunctionalTests
{
    public class ConnectionSpecificationTest
    {
        ...
        public void Can_specify_connection_string_in_OnConfiguring()
        {
            ...
            using (XGNorthwindContext.GetSharedStore())
            {
        ...
        private class StringInOnConfiguringContext : NorthwindContextBase
        {
            protected override void OnConfiguring(DbContextOptionsBuilder optionsBuilder)
                => optionsBuilder.UseXG(XGNorthwindContext.ConnectionString);
        }
        ...
        public void Can_specify_connection_in_OnConfiguring()
        {
            var serviceProvider
                = new ServiceCollection()
                    .AddScoped(p => new XGConnection(XGNorthwindContext.ConnectionString))
        ...
        public void Can_specify_connection_in_OnConfiguring_with_default_service_provider()
        {
            using (XGNorthwindContext.GetSharedStore())
            {
                using (var context = new ConnectionInOnConfiguringContext(new XGConnection(XGNorthwindContext.ConnectionString)))
                {
                    Assert.True(context.Customers.Any());
                }
            }
        ...
        private class ConnectionInOnConfiguringContext : NorthwindContextBase
        {
            ...
            protected override void OnConfiguring(DbContextOptionsBuilder optionsBuilder)
                => optionsBuilder.UseXG(_connection);
        ...
        {
            protected override void OnConfiguring(DbContextOptionsBuilder optionsBuilder)
                => optionsBuilder.UseXG("Database=Crunchie");
        }

        [Fact]
        public void Throws_if_no_connection_found_in_config_without_UseXG()
        {
            var serviceProvider
                = new ServiceCollection()
                    .AddDbContext<NoUseXGContext>().BuildServiceProvider();

            using (var context = serviceProvider.GetRequiredService<NoUseXGContext>())
            {
                Assert.Equal(
        ...
        [Fact]
        public void Throws_if_no_config_without_UseXG()
        {
            var serviceProvider
                = new ServiceCollection()
                    .AddDbContext<NoUseXGContext>().BuildServiceProvider();
        ...
        private class NoUseXGContext : NorthwindContextBase
        {
        }
        ...
        public void Can_depend_on_DbContextOptions_with_default_service_provider()
        {
            using (XGNorthwindContext.GetSharedStore())
            {
                using (var context = new OptionsContext(
                    ...
                    new XGConnection(XGNorthwindContext.ConnectionString)))
                {
                    Assert.True(context.Customers.Any());
                }
            }

...
namespace Microsoft.EntityFrameworkCore.XuGuDb.FunctionalTests
{
    public class ConnectionSpecificationTest
    {
        ...
        public void Can_specify_connection_string_in_OnConfiguring()
        {
            ...
            using (XuGuDbNorthwindContext.GetSharedStore())
            {
        ...
        private class StringInOnConfiguringContext : NorthwindContextBase
        {
            protected override void OnConfiguring(DbContextOptionsBuilder optionsBuilder)
                => optionsBuilder.UseXuGuDb(XuGuDbNorthwindContext.ConnectionString);
        }
        ...
        public void Can_specify_connection_in_OnConfiguring()
        {
            var serviceProvider
                = new ServiceCollection()
                    .AddScoped(p => new XGConnection(XuGuDbNorthwindContext.ConnectionString))
        ...
        public void Can_specify_connection_in_OnConfiguring_with_default_service_provider()
        {
            using (XuGuDbNorthwindContext.GetSharedStore())
            {
                using (var context = new ConnectionInOnConfiguringContext(new XGConnection(XuGuDbNorthwindContext.ConnectionString)))
                {
                    Assert.True(context.Customers.Any());
                }
            }
        ...
        {
            private readonly XGConnection _connection;

            public ConnectionInOnConfiguringContext(XGConnection connection)
            {
                _connection = connection;
            }

            protected override void OnConfiguring(DbContextOptionsBuilder optionsBuilder)
                => optionsBuilder.UseXuGuDb(_connection);
        ...
        [Fact]
        public void Throws_if_no_connection_found_in_config_without_UseXuGuDb()
        {
            var serviceProvider
                = new ServiceCollection()
                    .AddDbContext<NoUseXuGuDbContext>().BuildServiceProvider();

            using (var context = serviceProvider.GetRequiredService<NoUseXuGuDbContext>())
            {
                Assert.Equal(
        ...
        [Fact]
        public void Throws_if_no_config_without_UseXuGuDb()
        {
            var serviceProvider
                = new ServiceCollection()
                    .AddDbContext<NoUseXuGuDbContext>().BuildServiceProvider();
        ...
        private class NoUseXuGuDbContext : NorthwindContextBase
        {
        }
        ...
        public void Can_depend_on_DbContextOptions_with_default_service_provider()
        {
            using (XuGuDbNorthwindContext.GetSharedStore())
            {
                using (var context = new OptionsContext(
                    ...
                    new XGConnection(XuGuDbNorthwindContext.ConnectionString)))
                {
                    Assert.True(context.Customers.Any());
                }
            }
        ...
        private class OptionsContext : NorthwindContextBase
        {
            ...
            public OptionsContext(DbContextOptions<OptionsContext> options, XGConnection connection)
                : base(options)
            {
                _options = options;
                _connection = connection;

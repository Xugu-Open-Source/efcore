...
namespace Microsoft.EntityFrameworkCore.XuGuDb.FunctionalTests
{
    ...
    public class QueryNavigationsXuGuDbTest : QueryNavigationsTestBase<NorthwindQueryXuGuDbFixture>
    {
        [Fact]
...
SELECT `o`.`OrderID`, `o`.`CustomerID`, `o`.`EmployeeID`, `o`.`OrderDate`
 FROM `Orders` `o`",
        ...
        public override void Skip_Select_Navigation()
        {
            ...
            if (TestEnvironment.GetFlag(nameof(XuGuDbCondition.SupportsOffset)) ?? true)
            {
                Assert.StartsWith(
                    @":__p_0: 20
...
ORDER BY `od`.`Quantity`",
        ...
        public QueryNavigationsXuGuDbTest(NorthwindQueryXuGuDbFixture fixture, ITestOutputHelper testOutputHelper)
            : base(fixture)
        {

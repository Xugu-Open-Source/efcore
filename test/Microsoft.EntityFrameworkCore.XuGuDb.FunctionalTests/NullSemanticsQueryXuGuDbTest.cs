...
{
	public class NullSemanticsQueryXuGuDbTest : NullSemanticsQueryTestBase<XuGuDbTestStore, NullSemanticsQueryXuGuDbFixture>
    {
		public NullSemanticsQueryXuGuDbTest(NullSemanticsQueryXuGuDbFixture fixture)
            : base(fixture)
        {
        }
...
END",
 ...
 WHERE `e`.`NullableStringA` IS NULL",
		...
		public override void Compare_nullable_with_non_null_parameter_not_equal()
        {
            ...
            Assert.Equal(
                @":__prm_0: Foo (Size = 4000) (DbType = Object)
...
SELECT `e`.`Id`
 ...
 FROM `NullSemanticsEntity1` `e1`
INNER JOIN `NullSemanticsEntity2` `e2` ON `e1`.`NullableIntA` = `e2`.`NullableIntB`",
                Sql);
        }
 ...
 WHERE `e`.`NullableStringA` IN ('Foo') OR `e`.`NullableStringA` IS NULL",
		...
		public override void Where_multiple_ands_with_nullable_parameter_and_constant()
        {
            ...
            Assert.Equal(
                @":__prm3_2: Blah (Size = 8000) (DbType = AnsiString)
...
SELECT `e`.`Id`
 ...
 WHERE `e`.`NullableStringA` NOT IN ('Foo', :__prm3_2) AND `e`.`NullableStringA` IS NOT NULL",
		...
		public override void Where_multiple_ands_with_nullable_parameter_and_constant_not_optimized()
        {
            ...
            Assert.Equal(
                @":__prm3_2: Blah (Size = 4000) (DbType = Object)
...
SELECT `e`.`Id`
 ...
 WHERE `e`.`NullableBoolA` = `e`.`NullableBoolB`",
		...
		public override void Switching_parameter_value_to_null_produces_different_cache_entry()
        {
            ...
            Assert.Equal(
                @":__prm_0: Foo (Size = 8000) (DbType = AnsiString)

...
{
    public class InheritanceXuGuDbTest : InheritanceTestBase<InheritanceXuGuDbFixture>
    {
...
ORDER BY `a`.`Species`",
 ...
 FROM `Animal` `a`
 WHERE `a`.`Discriminator` = 'Kiwi' LIMIT 2",
                Sql);
        }
...
ORDER BY `e`.`Species` LIMIT 2

SELECT `a`.`Species`, `a`.`CountryId`, `a`.`Discriminator`, `a`.`Name`, `a`.`EagleId`, `a`.`IsFlightless`, `a`.`Group`, `a`.`FoundOn`
 FROM `Animal` `a`
 WHERE `a`.`Discriminator` IN ('Kiwi', 'Eagle') AND EXISTS (
    SELECT 1
     FROM `Animal` `e`
     WHERE (`e`.`Discriminator` = 'Eagle') AND (`a`.`EagleId` = `e`.`Species`) LIMIT 2)
ORDER BY `a`.`EagleId`",
                Sql);
        }
...
ORDER BY `c0`.`Name`, `c0`.`Id`",
 ...
 FROM `Animal` `b`
 WHERE (`b`.`Discriminator` = 'Kiwi') OR (`b`.`Discriminator` = 'Eagle')",
                Sql);
        }
...
SELECT COUNT(*)
 ...
 WHERE (`k`.`Discriminator` = 'Kiwi') AND `k`.`Species` LIKE '%' || 'owenii'",
        ...
        public InheritanceXuGuDbTest(InheritanceXuGuDbFixture fixture)
            : base(fixture)
        {
        }

...
{
    public class XuGuDbConfigPatternsTest
    {
                ...
                using (XuGuDbNorthwindContext.GetSharedStore())
                {
                    using (var context = new NorthwindContext(
                        new DbContextOptionsBuilder()
                            .UseXuGuDb(XuGuDbNorthwindContext.ConnectionString).Options))
                    {
                        Assert.Equal(91, await context.Customers.CountAsync());
                    }
                }
            ...
            public async Task Can_register_context_and_configuration_with_DI_container_and_have_both_injected()
            {
                var serviceProvider = new ServiceCollection()
                    ...
                    .AddSingleton(new DbContextOptionsBuilder()
                        .UseXuGuDb(XuGuDbNorthwindContext.ConnectionString).Options).BuildServiceProvider();
        ...
        public class ConstructorArgsToOnConfiguring
        {
            ...
            public async Task Can_pass_connection_string_to_constructor_and_use_in_OnConfiguring()
            {
                using (XuGuDbNorthwindContext.GetSharedStore())
                {
                    using (var context = new NorthwindContext(XuGuDbNorthwindContext.ConnectionString))
                    {
                        Assert.Equal(91, await context.Customers.CountAsync());
                    }
                }
